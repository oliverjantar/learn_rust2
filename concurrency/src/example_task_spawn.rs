...
async fn many_requests(requests: Vec<(String, u16, String)>) -> Vec<std::io::Result<String>> {
    ...
    for (host, port, path) in requests {
        ...
        // The future doesn't have to wait for the call block_on before it gets polled.
        // As soon as one of the threads from the thread pool is free, it will try polling it.
        // Future needs to be Send as it can be polled from a different thread.
        // Thread-local storage cannot be used in this case.
